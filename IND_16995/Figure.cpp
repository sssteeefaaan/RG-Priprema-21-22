...
PolyPoints Figure::GetPolyPoints(int n, DPOINT center, double r, double fi0, POINT mirror)
{
	...
	for (int i = 0; i < n; i++)
	{
		ret.points[i].x = int(center.x + mirror.x * (r * cos(i * dw + fi)) + 0.5);
		ret.points[i].y = int(center.y + mirror.y * (r * sin(i * dw + fi)) + 0.5);
	}
...
void Figure::Draw(CDC* pDC)
{
	...
	CPen* oldPen = pDC->SelectObject(new CPen(this->pen.style, this->pen.width, this->pen.color));
	COLORREF bkColor;
